...
export class SearchApi {
     ...
     * @param workspaceId 
     */
    public async searchGet (keyword?: string, workspaceId?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: SearchGet200Response;  }> {
        ...
        if (workspaceId !== undefined) {
            localVarQueryParameters['workspaceId'] = ObjectSerializer.serialize(workspaceId, "number");
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: SearchGet200Response;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "SearchGet200Response");
